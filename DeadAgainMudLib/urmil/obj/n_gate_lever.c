...
int openDoor(object who) {
	object door = load_object("/domains/northswamp/doors/urmil_gate_n.c");
	if(!door->GetClosed()){
		...
		tell_room("/domains/northswamp/room/gate1.c","The gate opens,");
		tell_room("/domains/urmil/room/n_gate2.c","The gate opens.");
	door->SetClosed(0);
	}
	return 1;
}
int closeDoor(object who) {
	...
	if(door->GetClosed()){
		...
		tell_room("/domains/urmil/room/n_gate2.c","The gate closes.");
		tell_room("/domains/northswamp/room/gate1.c","The gate closes.");
	door->SetClosed(1);
	}
	return 1;
}
